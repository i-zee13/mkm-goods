...
$sql = "SELECT id, (SELECT name from users where id = orders.employee_id) as name, po_num, total_amount, issue_date, invoice_num, currency, notes, terms, (SELECT company_name from customers where id = orders.customer_id) as company, (SELECT company_poc from customers where id = orders.customer_id) as poc, (SELECT country from customers where id = orders.customer_id) as country, (SELECT email from customers where id = orders.customer_id) as email from orders where id = ?";
...
$sql = "SELECT qty, product_desc, (SELECT description from product_related_items where id = order_contents.item_id) as item_description, weight_per_unit, weight_per_ctn, cbm, total_cbm, unit_price, amount, (SELECT name from product_related_items where id = order_contents.item_id) as product, (SELECT description from product_related_items where id = order_contents.item_id) as product_description from order_contents where order_id = ?";
$stmt = $conn->prepare($sql);
$performaId = $_GET['performa'];
$stmt->bind_param('i', $performaId);
$stmt->execute();
$stmt->bind_result($qty, $product_desc, $item_description, $weight_per_unit, $weight_per_ctn, $cbm, $total_cbm, $unit_price, $amount, $product, $product_description);
$contents = [];
while($stmt->fetch()){
    $contents[] = [ "qty" => $qty, "product_desc" => $product_desc, "item_description" => $item_description, "weight_per_unit" => $weight_per_unit, "weight_per_ctn" => $weight_per_ctn, "cbm" => $cbm, "total_cbm" => $total_cbm, "amount" => $amount, "unit_price" => $unit_price, "product" => $product, "product_description" => $product_description ];
}
...
$pdf->SetXY(168, 85);
$pdf->SetFont('Arial','',10);
$pdf->Cell(50, 5, $data["order"]["poc"],0,1);
...
$pdf->SetX(160);
$pdf->SetFont('Arial','',10);
$pdf->Cell(50, 5, $data["order"]["email"],0,1);
...
foreach ($data["contents"] as $item) {
    ...
    $pdf->SetFont('Arial','',9);
    $pdf->SetTextColor(64);
    $pdf->Cell(35, 5, $item["item_description"],0,0,'L');
