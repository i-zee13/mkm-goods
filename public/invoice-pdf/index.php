...
$stmt = $conn->prepare($sql);
$stmt->bind_param('i', $_GET['performa']);
$stmt->execute();
...
$customer_address[0]['street'] = "Dummy Address";
...
Class dcFpdf extends FPDF{
    ...
    function Header()
    {
        ...
        $this->SetXY(120, 51);
        $this->Cell(10, 4, "Expected Delivery Date: ",0,1,'L', 1);

        if($data['mode_of_shipment']){
            $this->SetX(120);
            $this->Cell(10, 4, "Mode of Shipment: ",0,1,'L', 1);
        }
        
        if($data['container_types']){
            $this->SetX(120);
            $this->Cell(10, 4, "Container Type: ",0,1,'L', 1);
        } 

        if($data['shipping_compnay']){
            $this->SetX(120);
            $this->Cell(10, 4, "Ship Via: ",0,1,'L', 1);
        }
        ...
        $this->SetXY(149, 51);
        $this->Cell(10, 4, $data['expected_delivery_date'],0,1,'L', 1);

        if($data['mode_of_shipment']){
            $this->SetX(143);
            $this->Cell(10, 4, $data['mode_of_shipment'],0,1,'L', 1);
        }
        
        if($data['container_types']){
            $this->SetX(140);
            $this->Cell(10, 4, "NA",0,1,'L', 1);
        } 

        if($data['shipping_compnay']){
            $this->SetX(132);
            $this->Cell(10, 4, $data['shipping_compnay'],0,1,'L', 1);
        }
...
foreach ($data["contents"] as $key => $item) {
    ...
    $pdf->MultiCell(102, 5, $item['product_description']."\n".($paramenter != '0' ? 'Product Details: '.$item['product_desc'] : ''),'R,L,B','L', 1);
...
$pdf->Cell(25, 5, "SUBTOTAL",1,0,'L', 1);
$pdf->Cell(25, 5, $data['currency_symbol'] .' ' .number_format($data['total_amount']),1,0,'L', 0);

$pdf->SetXY(122, $yPos+5); 
$pdf->Cell(25, 5, '',0,0,'C', 1);
$pdf->Cell(25, 5, "FREIGHT",1,0,'L', 1);
$pdf->Cell(25, 5, $data['currency_symbol'] .' ' .number_format($data['quoted_freight_charges']) ,1,0,'L', 0);

if($data['insurance_charges']){
    $yPos = $pdf->GetY(); 
    $pdf->SetXY(122, $yPos+5); 
    $pdf->Cell(25, 5, '',0,0,'C', 1);
    $pdf->Cell(25, 5, "INSURANCE",1,0,'L', 1);
    $pdf->Cell(25, 5, $data['currency_symbol'] .' ' .number_format($data['insurance_charges']) ,1,0,'L', 0);
}

if($data['advance']){
    $height_for_adv = $pdf->GetY();
    $pdf->SetXY(122, $height_for_adv+5); 
    $pdf->Cell(25, 5, '',0,0,'C', 1);
    $pdf->Cell(25, 5, "ADVANCE",1,0,'L', 1);
    $pdf->Cell(25, 5, $data['currency_symbol'] .' ' .number_format($data['advance']) ,1,0,'L', 0);
}

if($data['discount_value']){
    $height_for_dis = $pdf->GetY();
    $pdf->SetXY(122, $height_for_dis+5); 
    $pdf->Cell(25, 5, '',0,0,'C', 1);
    $pdf->Cell(25, 5, "DISCOUNT",'1',0,'L', 1);
    $pdf->Cell(25, 5, $data['currency_symbol'] .' ' .number_format($data['discount_value']) ,1,0,'L', 0);
}


$height_for_ttl = $pdf->GetY();
$total = (($data['total_amount'] + $data['quoted_freight_charges'] + $data['insurance_charges']) - $data['advance']) - $data['discount_value'];
$pdf->SetXY(122, $height_for_ttl+5); 
$pdf->SetFont('Arial','B',7);
$pdf->Cell(25, 5, '',0,0,'C', 1);
$pdf->Cell(25, 5, "TOTAL",'1',0,'L', 1);
$pdf->SetFont('Arial','B',7);
$pdf->Cell(25, 5, $data['currency_symbol'] .' ' .number_format($total),1,0,'L', 0);
...
$pdf->SetFont('rcb','',9);
$pdf->SetXY(10, 246); 
$pdf->Cell(20, 4, 'Country Of Origin: ',0,1,'L', 1);
 
$pdf->SetX(10); 
$pdf->Cell(40, 4, 'Port of Loading: ',0,1,'L', 1);   


$pdf->SetX(10); 
$pdf->Cell(40, 4, 'Discharge Port: ',0,1,'L', 1);


$pdf->SetX(10); 
$pdf->Cell(40, 4, 'Delivery Route: ',0,1,'L', 1);
...
$pdf->SetFont('cal','',9);
$pdf->SetXY(34, 246); 
$pdf->Cell(40, 4, $data['country_of_origin'],0,1,'L', 1);

$pdf->SetX(31); 
$pdf->Cell(40, 4, $data['port_of_loading'],0,1,'L', 1); 


$pdf->SetX(30.5); 
$pdf->Cell(40, 4, $data['port_of_discharge'],0,1,'L', 1);


$pdf->SetX(30.5); 
$pdf->Cell(40, 4, $data['delivery_routes'],0,1,'L', 1);
...
$pdf->SetXY(130, 215);
$pdf->Cell( 60, 60, $pdf->Image('../images/signature.PNG', 130, 210, 50), 0, 0, '', false );

$h = $pdf->GetY();
$pdf->Line(120, $h+40, 188, $h+40);

$pdf->SetXY(135, $h+43); 
$pdf->SetFont('Arial','B',11);
$pdf->Cell(40, 4, 'Export Manager',0,0,'C', 0);
