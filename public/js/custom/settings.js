...
$(document).ready(function () {
    ...
    $(document).on('click', '.openDataSidebarForAddingProperty', function () {
        ...
        $('#opp_name').text('Property Information');
        $('.designation_form_div').hide();
        $('.department_form_div').hide();
        $('.assets_form_div').hide();
        $('.custType_form_div').hide();
    ...
    $(document).on('click', '.openDataSidebarForUpdateProperty', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('input[name="property_name"]').val(response.property_name);
                $('input[name="property_name"]').blur();
...
function fetchAllData() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('.body_property').append('<table class="table table-hover dt-responsive nowrap" id="propertyTypeTable" style="width:100%;"><thead><tr><th>ID</th><th>Property</th><th>Action</th></tr></thead><tbody></tbody></table>');
            $('#propertyTypeTable tbody').empty();
            response.property_types.forEach(element => {
                $('#propertyTypeTable tbody').append(`<tr><td>${element['id']}</td><td>${element['property_name']}</td><td><button id="${element['id']}" class="btn btn-default btn-line openDataSidebarForUpdateProperty">Edit</button></td></tr>`);
            });
