...
$order_core = [];
while($stmt->fetch()){
    $order_core = [ "id" => $id, "cust_id" => $cust_id, "customer_name" => $customer_name, "customer_address" => $customer_address, "issue_date" => $issue_date, "po_num" => $po_num, "invoice_num" => $invoice_num, "total_amount" => $total_amount, 'currency' => $currency, 'country_of_origin' => $country_of_origin, 'invoice_to_address' => $invoice_to_address, 'shipment_route' => $shipment_route, 'invoice_to' => $invoice_to, 'container_types' => $container_types, 'paragraph' => $paragraph];
}
...
$pdf->MultiCell(85, 4, "QUALITY CONTROL CERTIFICATE" , 0, 'C');
$pdf->SetXY(85, 69);
$pdf->SetFont('Arial','B',11);
$pdf->Line(69, 69, 140, 69);
$pdf->SetXY(15, 85);
