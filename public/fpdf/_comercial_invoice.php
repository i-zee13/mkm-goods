...
$sql = "SELECT id, customer_id, currency_symbol, (Select company_name from customers where id = oc.customer_id) as customer_name, (Select address from customers where id = oc.customer_id) as customer_address, issue_date, po_num, invoice_num, total_amount, currency, order_discount, discount_value, eform_num, payment_type, (Select invoice_for from custom_invoices where order_id = oc.id) as invoice_for, (Select invoice_date from custom_invoices where order_id = oc.id) as invoice_date, (Select lc_number from custom_invoices where order_id = oc.id) as lc_number, (Select lc_date from custom_invoices where order_id = oc.id) as lc_date, (Select lc_through from custom_invoices where order_id = oc.id) as lc_through, vessel_name, vessel_number, shipment_route, (Select company_name from shipping_company where id = oc.shipment_company) as shipper_name, (Select invoice_to from custom_invoices where order_id = oc.id) as invoice_to, (Select invoice_address from custom_invoices where order_id = oc.id) as invoice_address, container_types from orders as oc where id = ?";
$stmt = $conn->prepare($sql);
$orderId = $_GET['oid'];
$stmt->bind_param('i', $orderId);
$stmt->execute();
$stmt->bind_result($id, $cust_id, $currency_symbol, $customer_name, $customer_address, $issue_date, $po_num, $invoice_num, $total_amount, $currency, $order_discount, $discount_value, $eform_num, $payment_type, $invoice_for, $invoice_date, $lc_number, $lc_date, $lc_through, $vessel_name, $vessel_number, $shipment_route, $shipper_name, $invoice_to, $invoice_address, $container_types);
$order_core = [];
while($stmt->fetch()){
    $order_core = [ "id" => $id, "cust_id" => $cust_id, "currency_symbol" => $currency_symbol, "customer_name" => $customer_name, "customer_address" => $customer_address, "issue_date" => $issue_date, "po_num" => $po_num, "invoice_num" => $invoice_num, "total_amount" => $total_amount, 'currency' => $currency, 'order_discount' => $order_discount, 'discount_value' => $discount_value, 'eform_num' => $eform_num, 'payment_type' => $payment_type, 'invoice_for' => $invoice_for, 'invoice_date' => $invoice_date, 'lc_number' => $lc_number, 'lc_date' => $lc_date, 'lc_through' => $lc_through, 'vessel_name' => $vessel_name, 'vessel_number' => $vessel_number, 'shipment_route' => $shipment_route, 'shipper_name' => $shipper_name, 'invoice_to' => $invoice_to, 'invoice_address' => $invoice_address, 'container_types' => $container_types];
}
$data = $order_core;
$stmt->close();

// echo "<pre>"; print_r($order_core);die;
...
$pdf->SetXY(10, 65);
$pdf->SetFont('Arial','B',8);
$pdf->Cell( 40, 4, "Consigned to Messers: ".($data['invoice_to'] ? $data['invoice_to'] : $data['customer_name']). ' - ' .($data['invoice_address'] ? $data['invoice_address'] : $data['customer_address']), 0, 'L');

$pdf->SetXY(10, 75);
$pdf->SetFont('Arial','B',8);
$pdf->Cell( 40, 4, 'against Purchase Order No/Lc '.($data['lc_number'] ? $data['lc_number'] . ' ' . $data['lc_date'] : $data['po_num'] ), 0, 'L');

$pdf->SetXY(10, 79);
$pdf->SetFont('Arial','B',8);
$pdf->Cell( 40, 4, $start_shipment_port.'/'.$end_shipment_port, 0, 'L');
...
if(!in_array('goods', $parameters)){
    ...
    if(in_array('amount', $parameters))
        $pdf->Cell((190/sizeof($parameters)), 9, "AMOUNT IN Rs.",1,0,'C');
}else{
    ...
    if(in_array('amount', $parameters))
        $pdf->Cell(ROUND((10.52/100)*190, 2), 9, "AMOUNT IN Rs.",1,0,'C');
}
...
foreach ($data["contents"] as $key => $item) {
    ...
    if(!in_array('goods', $parameters)){
        ...
        if(in_array('marks', $parameters))
            $pdf->Cell((190/sizeof($parameters)), 9, $key+1,1,0,'C');
    
        if(in_array('qty', $parameters))
            $pdf->Cell((190/sizeof($parameters)), 9, $item['qty'],1,0,'C');
    
        if(in_array('price', $parameters))
            $pdf->Cell((190/sizeof($parameters)), 9, $item['commercial_unit_price'],1,0,'C');
    
        if(in_array('amount', $parameters))
            $pdf->Cell((190/sizeof($parameters)), 9, number_format($item['commercial_amount']),1,0,'C');
        ...
        if(in_array('price', $parameters))
            $pdf->Cell(ROUND((10.52/100)*190, 2), 9, $item['commercial_unit_price'],1,0,'C');
    
        if(in_array('amount', $parameters))
            $pdf->Cell(ROUND((10.52/100)*190, 2), 9, number_format($item['commercial_amount']),1,0,'C');
    }
...
//     $containerNumbers = json_decode($data['container_types'],true);
...
//             $pdf->SetXY(65, $yPos);
//             $pdf->SetFont('Arial','B',8);
//             $pdf->Cell(85, 4, 'CONTAINER # '.$value['number'] , 0);
//             if($yPos < 280){
//                 $yPos += 5;
//             }
//         }
...
$totalColumnHeight = (sizeof($containerNumbers)*5)+15;
// $containerNumbers = [];
if(in_array('goods', $parameters)){
    $pdf->SetXY(10, $pdf->GetY()+9);
    if(in_array('marks', $parameters))
    ...
    $pdf->SetX($xPos);
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell((190 - $ttlWidthCovered), 4, '', 'B', 1);
    $pdf->SetFontSpacing(0);

    $pdf->SetFont('Arial','',6);
    $pdf->SetXY($xPos+(190 - $ttlWidthCovered), $yPos);
    if(in_array('price', $parameters))
        $pdf->Cell(ROUND((10.52/100)*190, 2), $totalColumnHeight, 'UP',1,0,'C');
    if(in_array('amount', $parameters))
        $pdf->Cell(ROUND((10.52/100)*190, 2), $totalColumnHeight, 'CommAmm',1,0,'C');
}else{
    $pdf->SetXY(10, $pdf->GetY()+9);
    if(in_array('marks', $parameters))
    ...
    if(in_array('qty', $parameters))
        $pdf->Cell((190/sizeof($parameters)), $totalColumnHeight, 'qty',1,0,'C');

    if(in_array('price', $parameters))
        $pdf->Cell((190/sizeof($parameters)), $totalColumnHeight, 'UP',1,0,'C');

    if(in_array('amount', $parameters))
        $pdf->Cell((190/sizeof($parameters)), $totalColumnHeight, 'commercial_amount',1,0,'C');
}
// Total Row End
...
// $pdf->MultiCell(25, ($H == $yPos ? $yPos : ($H - $yPos) / 2), number_format($total_ctn).'CTNS' ,0,"C");
...
// $pdf->SetXY(170, $yPos);
// $pdf->SetFont('Arial','B',7);
// $pdf->MultiCell(30, ($H == $yPos ? $yPos : $H - $yPos), $data['order_discount'].'%',0,'C');
...
// $pdf->SetFont('Arial','B',7);
// $pdf->MultiCell(30, ($H == $yPos ? $yPos : $H - $yPos), ($data['currency'] == 'USD' ? "$ " : 'PKR ').number_format($total_amount - $data['discount_value']),0,'C');
