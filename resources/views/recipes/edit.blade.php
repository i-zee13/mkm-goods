...
@section('data-sidebar')
    ...
    <div id="product-cl-sec"> <a href="#" id="pl-close" class="close-btn-pl"></a>
        <div class="pro-header-text">Add <span>Items</span></div>
                                            ...
                                            @foreach($recipes as $recipe)
                                                <tr>
                                                    <td width="80%"><div class="ItemsListDiv">
                                                            <div id="{{$recipe->id}}" class="PR_Name">{{$recipe->item_recipe_name}}</div>
                                                        </div></td>
                                                    <td><button id="ingAdd_{{$recipe->id}}" onclick="addIng('{{$recipe->id}}','{{$recipe->item_recipe_name}}');" class="btn btn-default mb-0">Add</button></td>
                                                </tr>
                                            @endforeach
            ...
            <div class="card">
                        ...
                        <div class="row m-0">
                            ...
                            <div class="col-3 pr-0">
                                <div class="form-group">
                                    <label class="control-label mb-10">Recipe Name</label>
                                    <input type="text" id="recipe_name" name="recipe_name" value="{{$r_name}}" class="form-control" placeholder="">
                                </div>
                            </div>

                            <div class="col-3 pr-0">
                                <button id="productlist01" type="submit" class="btn btn-primary mt-19">Add Items</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="editBody p-20">
                            <table class="table table-hover nowrap editItemsTable" style="width:100%;">
                                <thead>
                                <tr>
                                    <th>id</th>
                                    <th>Item Name</th>
                                ...
                                @foreach($recipeData as $data)
                                    <tr>
                                        <td>{{$data->item_id}}</td>
                                        <td>{{$data->item_recipe_name}}</td>
                                        <td><button type="button" name="{{$data->item_recipe_name}}" id="{{$data->item_id}}" class="btn btn-default red-bg removeIngBtnGrid" title="Delete">Remove</button></td>
                                    </tr>
                                @endforeach
                        ...
                        <div class="col-md-12 p-0">
                            <div style="background-color: #f6f6f6; padding: 15px; margin-top: 15px; margin-bottom: 0px; text-align: center; margin-bottom: 1px">
                                <button type="button" class="btn btn-primary mr-2 saveIngForm">Save</button>
                                <button id="pl-close" type="submit" class="btn btn-cancel mr-2">Cancel</button>
                            </div>
                        </div>
                    </div>
